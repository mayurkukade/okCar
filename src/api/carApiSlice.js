  ...
  endpoints: (builder) => ({
    ...
    filterCar: builder.query({
      query: (
        ...
        console.log(
          `Filter Api Slice ${currentPage}?minPrice=${minPrice}&maxPrice=${maxPrice}&area=${area}&year=${year}&brand=${brand}&model=${model}&transmission=${transmission}&fuel_type=${fuelType}`
        );
        return {
